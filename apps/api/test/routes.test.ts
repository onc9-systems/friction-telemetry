...
describe("routes", () => {
  ...
  // ping, document-index and its onFailure handler (served as a function of its own), event-answered, event-still-stuck.
  it("given GET /api/inngest, then the Inngest serve handler lists every function", async () => {
    const res = await call("GET", "/api/inngest");
    expect(res.status).toBe(200);
    expect(await res.json()).toMatchObject({ function_count: 5 });
  });
