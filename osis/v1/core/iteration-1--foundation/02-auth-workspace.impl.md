...
### Engineering Notes: fixed identity before sign-in (25 September 2026)

Andrés asked for the app on three Macs against one production service, with three fixed people and a switcher, before this spec is built. Branch `iteration-1/fixed-identity`. It builds this spec's seams with a fixed directory behind them, so this spec later replaces only the inside of `requireActor` and the Mac's `Identity`.

- **People.** `packages/contracts/fixtures/fixed-directory.json`, schema `Directory`: organization `org_acme` (Acme Logistics, the fixtures' organization) and `usr_andres` Andrés Campos (Project Manager, leader), `usr_romina` Romina (employee). The service and the Mac read the same file. Amir (Executive, leader) was removed on 25 September 2026 when the demo narrowed to two people; a Mac still set to him falls back to Romina.
- **Service.** `src/auth/actor.ts` `requireActor` on `/v1/*` except `/v1/health`: reads `x-ft-user`, 401 `actor_required` when absent, 401 `unknown_person` when not in the directory, sets `c.var.actor = { userId, organizationId, roles }` exactly as this spec names it. `src/auth/gates.ts` `requireRole` and `LEADER_ROUTES` (the list in this spec, plus `GET /v1/people`), applied centrally in `index.ts`, so route files stay untouched. `GET /v1/me` returns `Me`; `GET /v1/people` returns the directory. Routes read the actor through `AppEnv` (`Hono<AppEnv>`). Hono middleware runs only for handlers registered after it (Context7 `/honojs/website`, middleware guide), so identity and gates are registered before every route; a test kills the reordering.
- **Mac.** `Client/Identity.swift`: the chosen person in UserDefaults `identity.userId`; `URLRequest.identify()` adds the header, and every request to the service must call it. A Mac never set acts as the first person without leader permission. `AppModel.switchPerson(to:)` changes the header, the sidebar (leader permission follows the person), and clears what this session sent as the previous person. Switchers in the menu bar ("Using Friction as") and Settings.
- **Not secured.** Anyone who knows the URL can send `x-ft-user`. Acceptable for a demo on sample data; this spec's JWT verification is the fix.
- **For other branches.** Replace `SHELL_ACTOR` with `c.var.actor` (type `AppEnv`), and call `request.identify()` on every Mac request, or the service answers 401.
