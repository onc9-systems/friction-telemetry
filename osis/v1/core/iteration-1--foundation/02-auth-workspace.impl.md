...
- **People.** `packages/contracts/fixtures/fixed-directory.json`, schema `Directory`: organization `org_acme` (Acme Logistics, the fixtures' organization) and `usr_andres` Andrés Campos (Project Manager, leader), `usr_romina` Romina (employee), `usr_amir` Amir (Executive, leader). The service and the Mac read the same file. PM and Executive are titles, not roles: both are leaders in `product.md` terms.
