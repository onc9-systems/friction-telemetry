...
| Concurrency `[{ key: "event.data.organizationId", limit: 3 }, { scope: "account", key: "\"workers-ai\"", limit: 5 }]` (was 10; the Inngest plan caps limits at 5, 25 Sep 2026) | Per-organization fairness plus one account-wide virtual queue any Workers AI function can join (Inngest concurrency guide, Context7) | Throttle: limits starts, not in-flight steps |
